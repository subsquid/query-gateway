...
async fn fetch_remote_file(filename: &str) -> anyhow::Result<DatasetList> {
    let url = format!(
        "https://storage.googleapis.com/subsquid-public/archives/{}",
        filename
    );
    tracing::debug!("Loading remote file from {}", url);
...
async fn load_file(filename: &str) -> anyhow::Result<DatasetList> {
    if let Ok(path) = env::var("DATASET_DATA_PATH") {
        ...
        if full_path.exists() {
            ...
            Ok(serde_json::from_reader(reader)
                .with_context(|| format!("failed to parse dataset {}", full_path.display()))?)
        } else {
            tracing::debug!(
            ...
            fetch_remote_file(filename).await
        }
    } else {
        fetch_remote_file(filename).await
    }
...
pub async fn datasets_info_load() -> anyhow::Result<DatasetList> {
    ...
    let all_datasets: Vec<Dataset> = config_evm
        .datasets
        .into_iter()
        .map(|mut d| {
